...
This test is technically correct, but can be quite hard to read if you are not familiar with HTML (e.g. you might not
know what the HTML attribute `id` is). There is a lot of detail in the test case that is not relevant to the test case
itself and may distract you from understanding what its purpose is.

We can start refactoring our test by adding the `*** Keywords ***` table to our test case file and creating a custom
keyword for opening the browser to the login page.
...
### Exercise 3.1: Refactor using user defined keywords

Refactor the rest of the test case by creating the following user defined keywords:
...
_Hint:_ One user defined keyword can contain multiple keywords, just like a test case can have
...
_NOTE:_ If you use `=` between the variable name and value, ensure you use **1 space at maximum** between the
...
_Hint:_ You can combine variables with text like this `${URL}/` or `${URL}/welcome.html` without
...
_Hint:_ If you don't want to actually run the test case but just check the syntax you can also run
`robot --dryrun robot/login.robot`. Check `robot -h` for more details.

Now that you've successfully changed to use `${URL}` variable

### Exercise 3.2: Refactor using variables

Think about what other values could
...
- Consider what other values could be implemented as variables, add them to your variables table and
  replace hard-coded values in your test case or keyword with the variable. - For example the credentials and/or different locators could be implemented as variables.
