...
Test Setup is a keyword that is executed before a test case.
It is something like a precondition that needs to be fulfilled in order for the rest of the test case to work.
For example, many test cases require a user to be logged in, so we could have a Test Setup that logs user in.

We can denote a keyword as a setup
by adding `[Setup]` before it or by adding it to the `*** Settings ***` table.
...
_Hint:_ `#` starts a comment line for Robot Framework. Nothing after it (on that line) will be executed.
