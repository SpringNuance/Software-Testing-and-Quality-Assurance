...
## Exercise 5.1: First negative test

- In the file `invalid_login.robot` under `robot` folder, create a test case `Error Page Should Be Visible After Incorrect Login` into the `invalid_login.robot`.
...
- Create a keyword called `Verify That Error Page Is Visible` to check we've landed on the error page. For now, it is sufficient to use one
  combination of invalid credentials.

Run the test with `robot robot/invalid_login.robot` command to verify the results.

## Exercise 5.2: Combitational Testing

Think about invalid combinations of a username and a password, and write them down. Make sure you get at least six different combinations. Think about how you would test all of them with the tools we have so far.

_Hint_: Checkout also Robot Framework [built-in variables](http://robotframework.org/robotframework/latest/RobotFrameworkUserGuide.html#built-in-variables).

---

When thinking through testing the different combinations, you may have noticed that creating a new keyword for entering credentials in each
...
### Exercise 5.3: Keyword arguments

- Define keyword arguments to `Enter Username` and `Enter Password` by putting `[Arguments]` after the
  keyword name. - Add 4 spaces and a **variable** after `[Arguments]`.
- Replace the hard-coded value with your new argument.

NOTE: If you have a `${USERNAME}` variable in the `*** Variables ***` table, and also use
...
_Hint:_ The `Fill Secret` keyword might fail with an error saying `Invalid variable name ''`. This
