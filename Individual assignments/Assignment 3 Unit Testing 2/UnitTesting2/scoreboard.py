...
class Scoreboard:
    ...
    def add_score(self, name, score, difficulty):
        is_valid_score = score is not None and score > 0  # A: Score is valid
        is_valid_name = name and isinstance(name, str)  # B: Name is valid
        is_valid_difficulty = difficulty in ["easy", "hard"]  # C: Difficulty is valid
        
        if is_valid_score and is_valid_name and is_valid_difficulty:
            self.scores.append((name, score, difficulty))
            self.scores = sorted(self.scores, key=lambda x: x[1], reverse=True)
            self.save_scores()
        else:
            raise ValueError("Invalid input for adding score.")
